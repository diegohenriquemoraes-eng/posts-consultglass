# -*- coding: utf-8 -*-
"""Publica o carrossel técnico do dia no feed do @peritomauricioboschetti.

Roda SEGUNDA e QUINTA, 7h BRT (workflow `carrossel.yml`). Fluxo — o mesmo do
posts-perffec, que é o mesmo da mini-aula do @vendanaobra:
  ...
  3. commita e sobe as imagens — raw.githubusercontent é a URL pública que a
     API exige (por isso o repo é PÚBLICO; nada de segredo entra nele)
...
Uso:
    python publicar.py              # próxima da sequência (só seg/qui)
...
Token: META_TOKEN (secret do Actions) ou Perffec\\Claude\\meta_token_mauricio.txt.
A conta do Maurício NÃO está no portfólio empresarial do Diego, então o caminho
é a **API do Instagram com login do Instagram** (`api: "instagram"` no config):
token de 60 dias em graph.instagram.com, renovável por `refresh_access_token`.
Se um dia a conta for ligada a uma Página acessível pelo app vendanaobra, basta
trocar `api` para "facebook" e usar um token de usuário como no posts-perffec.
"""
...
DIAS = (0, 3)  # segunda, quinta
USERNAME = "peritomauricioboschetti"
...
API_MODO = CFG.get("api", "instagram")
API = ("https://graph.instagram.com/v21.0" if API_MODO == "instagram"
...
def _token() -> str:
    ...
    if tok:
        return tok
    if os.path.exists(TOKEN_LOCAL):
        with open(TOKEN_LOCAL, encoding="utf-8") as f:
            return f.read().strip()
    raise SystemExit("Sem token: defina META_TOKEN ou salve meta_token_mauricio.txt")


def _ig_user_id() -> str:
    return os.environ.get("IG_USER_ID", "").strip() or CFG.get("ig_user_id") or ""
...
def _post(endpoint: str, campos: dict) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"API falhou em {endpoint}: {corpo}")
...
def _commitar(mensagem: str, *caminhos: str) -> None:
    ...
    if pronto:
        _git("-c", "user.name=consultglass-bot", "-c", "user.email=bot@consultglass.net.br",
             "commit", "-m", mensagem)
    _git("push", "origin", "main")


def esperar_container(container_id: str, token: str, tentativas: int = 30) -> None:
    """A API precisa baixar a imagem antes de deixar publicar."""
...
def conferir_token(token: str, ig_id: str) -> None:
    ...
    r = _get(ig_id or "me", {"fields": "username,followers_count", "access_token": token})
    if r.get("username") != USERNAME:
        raise SystemExit(f"token aponta para @{r.get('username')} — não é o Maurício; abortando")
    _log(f"token ok — @{r['username']}, {r.get('followers_count', '?')} seguidores")
    if API_MODO == "instagram":
        # graph.instagram.com não tem debug_token; a validade é acompanhada por config.json
        vence = CFG.get("token_vence_em")
        if vence:
            dias = (datetime.fromisoformat(vence) - datetime.now()).days
            _log(f"token vence em ~{dias} dias ({vence})")
            if dias < 10:
                raise SystemExit(f"TOKEN VENCE EM {dias} DIAS — rodar `python renovar_token.py` no PC do Diego")
    else:
        ...
        if exp:
            ...
            if dias < 10:
                raise SystemExit(f"TOKEN VENCE EM {dias} DIAS — renovar no PC do Diego")
...
def main() -> None:
    ...
    if a.slug is None and not a.ensaio and agora.weekday() not in DIAS:
        _log("carrossel só sai segunda e quinta — nada a fazer")
        return
    ...
    ig_id = _ig_user_id()
    if not ig_id:
        raise SystemExit("Sem ig_user_id: preencher config.json (id da conta @peritomauricioboschetti)")
